...
export const VideoActor = (props) => {
  return (
    <div className='flex flex-col'>
      <VideoTabs active='actor' />
      <ul className='grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8'>
        {actors.map((actor) => (
          <li key={actor.name} className='relative'>
            <div className={clsx('group block w-full aspect-w-10 aspect-h-7 rounded-lg bg-gray-100 overflow-hidden', actor.active && 'ring-2 ring-offset-2 ring-offset-gray-100 ring-indigo-500')}>
              <img
                src={actor.picture}
                alt=''
